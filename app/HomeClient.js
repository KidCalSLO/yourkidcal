...
const CATEGORY_LABELS = {Rec:'Recreation'}
...
const SORTS = [['deadline','Deadline (soonest)'],['program','Program date (soonest)']]
const WHEN_RANGES = [['any','Any time'],['30','Next 30 days'],['90','Next 3 months'],['past90','Beyond 3 months']]
...
export default function HomeClient({listings}) {
  ...
  const [whenRange, setWhenRange] = useState('any')
...
const filtered = sorted.filter(l => {
  ...
  const matchWhen = whenRange==='any' || (!l.is_rolling && (
    (whenRange==='30' && days>=0 && days<=30) ||
    (whenRange==='90' && days>=0 && days<=90) ||
    (whenRange==='past90' && days>90)
  ))
  ...
  function clearFilters() {
    ...
    setAgeMin(0); setAgeMax(18); setSearch(''); setWhenRange('any')
  }
  ...
  return (
    <>
      ...
      {/* ── TABS ── */}
      <div style={s.tabBar}>
        {[['browse','Browse'],['reviews','Reviews']].map(([id,label])=>(
      ...
      {tab==='browse'&&(
        <>
        <div style={{maxWidth:1280,margin:'0 auto',padding:isMobile?'0.85rem 1rem 0':'1rem 2rem 0'}}>
          <div style={{position:'relative',maxWidth:420}}>
            <input
              style={{border:'1.5px solid #e0ddd5',borderRadius:10,padding:'9px 12px 9px 34px',fontFamily:"'DM Sans',sans-serif",fontSize:13,background:'#fff',outline:'none',width:'100%',boxSizing:'border-box'}}
              placeholder="Search programs, orgs, areas..."
              value={search}
              onChange={e=>setSearch(e.target.value)}
            />
            <span style={{position:'absolute',left:11,top:'50%',transform:'translateY(-50%)',fontSize:13,color:'#888780'}}>🔍</span>
          </div>
        </div>
          ...
          {!isMobile&&(
            <div style={{width:220,flexShrink:0,position:'sticky',top:64,maxHeight:'calc(100vh - 64px)',overflowY:'auto',padding:'1.5rem 1.25rem 1.5rem 0',borderRight:'1.5px solid #e0ddd5'}}>
              ...
              <div style={{marginBottom:'1.25rem'}}>
                ...
                {CATEGORIES.map(cat=>(
                  <button key={cat} className="side-btn" onClick={()=>setFilter(f=>f===cat?'All':cat)}
              ...
              <div style={{marginBottom:'1.25rem'}}>
                ...
                {LOCATIONS.map(loc=>(
                  <button key={loc} className="side-btn" onClick={()=>setLocation(l=>l===loc?'All Areas':loc)}
          ...
          {isMobile&&(
            <div style={{background:'#fff',borderBottom:'1.5px solid #e0ddd5',width:'100%'}}>
              <div className="filter-scroll" style={{padding:'.4rem 1rem',borderBottom:'1px solid #f0ede6'}}>
                ...
                {CATEGORIES.map(cat=>(
                  <button key={cat} style={{...s.chip(filter===cat),marginRight:6}} onClick={()=>setFilter(f=>f===cat?'All':cat)}>{catLabel(cat)}</button>
                ))}
              </div>
              ...
              <div className="filter-scroll" style={{padding:'.3rem 1rem',borderBottom:'1px solid #f0ede6'}}>
                ...
                {LOCATIONS.map(loc=>(
                  <button key={loc} style={{...s.chip(location===loc,'#185FA5'),marginRight:6}} onClick={()=>setLocation(l=>l===loc?'All Areas':loc)}>{loc}</button>
                ))}
              </div>
          ...
          <div style={{flex:1,minWidth:0,padding:isMobile?'1rem':'1.5rem 0 1.5rem 1.75rem'}}>
            ...
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem',flexWrap:'wrap',gap:8}}>
              ...
              <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                <select aria-label="Sort programs" value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{border:'1.5px solid #e0ddd5',borderRadius:8,padding:'6px 10px',fontSize:12,fontFamily:"'DM Sans',sans-serif",color:'#2C2C2A',background:'#fff',cursor:'pointer'}}>
                  {SORTS.map(([id,label])=><option key={id} value={id}>Sort: {label}</option>)}
                </select>
                <select aria-label="Filter by timeframe" value={whenRange} onChange={e=>setWhenRange(e.target.value)} style={{border:'1.5px solid #e0ddd5',borderRadius:8,padding:'6px 10px',fontSize:12,fontFamily:"'DM Sans',sans-serif",color:'#2C2C2A',background:'#fff',cursor:'pointer'}}>
                  {WHEN_RANGES.map(([id,label])=><option key={id} value={id}>{label}</option>)}
                </select>
                <button onClick={()=>setHideClosed(h=>!h)} style={{background:hideClosed?'#2C2C2A':'#fff',color:hideClosed?'#fff':'#2C2C2A',border:'1.5px solid #e0ddd5',borderRadius:20,padding:'5px 12px',fontSize:11,fontWeight:500,cursor:'pointer'}}>
                  {hideClosed?'Show closed programs':'Hide closed programs'}
                </button>
              </div>
            </div>
                ...
                return (
                  <div key={l.id} className="listing-card" style={{
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:8}}>
                      ...
                      <button aria-label={isSaved(l.id)?'Remove from saved list':'Save to my list'} title={isSaved(l.id)?'Remove from saved list':'Save to my list'} style={{background:isSaved(l.id)?'#EAF3DE':'none',border:isSaved(l.id)?'1.5px solid #3B6D11':'1.5px solid #e0ddd5',color:isSaved(l.id)?'#3B6D11':'#888780',borderRadius:6,width:28,height:28,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',fontSize:13,flexShrink:0}} onClick={()=>toggleSave(l)}>🔖</button>
                    </div>
                    ...
                    <div style={{display:'flex',flexWrap:'wrap',gap:'5px 12px',fontSize:11,color:'#888780'}}>
                      {!past&&(
                        <span title="Registration deadline" style={{color:urgencyColor(days),fontWeight:600}}>
                          {l.is_rolling ? '↻ Rolling enrollment' : `⏰ Deadline ${formatDateShort(l.reg_close||l.deadline)} · ${days}d left`}
                        </span>
                      )}
                      {past&&<span>🔒 Closed</span>}
                      {l.program_start&&<span title="Program dates">▶ {formatDateShort(l.program_start)}{l.program_end?` – ${formatDateShort(l.program_end)}`:''}</span>}
                      {l.ages&&<span title="Age range">👥 Ages {l.ages}</span>}
                      {l.cost_free
                        ...
                        : (l.cost===null||l.cost===undefined)
                          ? <span title="Price">💰 Price varies</span>
                          : <span title="Price">💰 ${l.cost.toLocaleString()}</span>
                      }
                    </div>
                    ...
                    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',paddingTop:8,borderTop:'1px solid #f0ede6',marginTop:'auto'}}>
                      <button aria-label="Add to calendar" title="Add to calendar" style={{background:'none',border:'1.5px solid #e0ddd5',color:'#888780',borderRadius:6,padding:'0 10px',height:28,display:'flex',alignItems:'center',justifyContent:'center',gap:4,cursor:'pointer',fontSize:11,whiteSpace:'nowrap'}} onClick={()=>setCalModal(l)}>📅 Add to calendar</button>
  ...
  ? <a href={l.registration_url} target="_blank" rel="noopener noreferrer" className="register-btn"
      ...
      {l.is_rolling?'Enroll on their site →':'Register on their site →'}
    </a>
  : past
          ...
          <div style={s.submitModal} onClick={e=>e.stopPropagation()}>
            ...
            {notifySubmitted?(
              ...
              <>
                ...
                {[['Email *','email','email','your@email.com'],['Name','name','text','e.g. Sarah']].map(([label,key,type,ph])=>(
                ...
                <div style={{marginBottom:'.85rem'}}>
                  ...
                  <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
                    {['Camp','School','Sport','Daycare','Rec','Arts'].map(cat=>(
                      <button key={cat} onClick={()=>toggleNotifyCat(cat)} style={{border:notifyForm.categories.includes(cat)?'none':'1.5px solid #e0ddd5',background:notifyForm.categories.includes(cat)?'#2C2C2A':'#fff',color:notifyForm.categories.includes(cat)?'#fff':'#2C2C2A',borderRadius:20,padding:'5px 12px',fontSize:12,cursor:'pointer'}}>{catLabel(cat)}</button>
                    ))}
                  </div>
                </div>
                <div style={{marginBottom:'.85rem'}}>
              ...
              <>
                ...
                <div style={{marginBottom:'.85rem'}}>
                  ...
                  <select style={s.input} value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))}>
                    {['Camp','School','Sport','Daycare','Rec','Arts'].map(c=><option key={c} value={c}>{catLabel(c)}</option>)}
                  </select>
                </div>
